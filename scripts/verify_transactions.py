#!/usr/bin/env python3
"""Verify the 100-transaction dataset processes correctly end-to-end.
...
Usage: python3 scripts/verify_transactions.py
...
BALANCES_CSV = "data/mable_account_balances_50.csv"
TRANSACTIONS_CSV = "data/mable_transactions_100.csv"
...
for row in rows:
    ...
    if amount > balances[frm]:
        ...
        blocked_set.add(frm)  # blocked for the rest of the batch; balance untouched
        continue
    balances[frm] -= amount
    balances[to] += amount
...
with open(TRANSACTIONS_CSV, "rb") as f:
    ...
    body = b"".join([
        f"--{boundary}\r\n".encode(),
        b'Content-Disposition: form-data; name="file"; filename="mable_transactions_100.csv"\r\n',
...
if len(actual) != 100:
    failures.append(f"transaction count: {len(actual)} (expected 100)")
...
if actual != expected:
    for i, (a, e) in enumerate(zip(actual, expected)):
        ...
        failures.append("transaction list length mismatch")

counts = {}
for status, reason in actual:
    key = reason or status
    counts[key] = counts.get(key, 0) + 1
expect_counts = {"complete": 95, "INSUFFICIENT_BALANCE": 3, "ACCOUNT_NOT_FOUND": 2}
...
print("=" * 60)
print("VERIFICATION REPORT — 100 transactions")
print("=" * 60)
